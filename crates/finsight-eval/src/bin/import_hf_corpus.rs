...
fn main() -> Result<()> {
    ...
    let input = args
        .next()
        .context("usage: import_hf_corpus <input.csv> <output.jsonl>")?;
    let output = args
        .next()
        .context("usage: import_hf_corpus <input.csv> <output.jsonl>")?;
    ...
    let mut brand_categories: BTreeMap<String, std::collections::BTreeSet<String>> =
        BTreeMap::new();
    ...
    out.push_str(
        "// Generated by `cargo run -p finsight-eval --bin import_hf_corpus`. Do not hand-edit.\n",
    );
    ...
    for (i, line) in lines.enumerate() {
        ...
        if ambiguous.contains(&merchant_id) {
            *dropped
                .entry(format!("<ambiguous brand: {merchant_id}>"))
                .or_insert(0) += 1;
            continue;
        }
    ...
    eprintln!(
        "kept {kept} rows across {} merchants -> {output}",
        per_merchant.len()
    );
