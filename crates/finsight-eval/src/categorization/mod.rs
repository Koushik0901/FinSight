...
//! - [`private_eval`] — the REFRAMED, local-only alternative to issue #89's
//!   literal ask: reuses `split`/`confusion`/`predictors` unmodified, but
//!   over a self-hosted instance's own real `categorizations` corrections
//!   instead of a checked-in synthetic/real corpus file. Never wired into
//!   `bindings.ts` or the Copilot surface — see that module's doc comment.
//!
//! ## What this is NOT
