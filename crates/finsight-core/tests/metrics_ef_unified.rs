...
fn emergency_fund_months_unified() {
    ...
    let chk = accounts::insert(&mut conn, acct("chk", 500_000, true))
        .unwrap()
        .id;
    let _brk = accounts::insert(&mut conn, acct("brk", 200_000, false))
        .unwrap()
        .id;
    ...
    let expense_acct = accounts::insert(&mut conn, acct("expense", 0, false))
        .unwrap()
        .id;
    let now = Utc::now();
    for m in 0..3 {
        let d = (now - Duration::days(10 + m * 32))
    ...
    assert!(
        (ra.emergency_fund_months - m).abs() < 0.01,
        "rolling_averages.emergency_fund_months {} vs {m}",
        ra.emergency_fund_months
    );
}
...
fn rolling_uses_ef_eligible_not_total_liquid() {
    ...
    .unwrap();
    let expense_acct = accounts::insert(&mut conn, acct("expense2", 0, false))
        .unwrap()
        .id;
    let now = Utc::now();
    for m in 0..3 {
    ...
    assert!(
        m < 3.0,
        "must use EF-eligible pool, not total liquid; got {m}"
    );
}
...
fn ef_pool_ignores_foreign_currency_and_unknown_balance() {
    ...
    assert!(
        (m - 3.0).abs() < 0.05,
        "EF pool must be CAD-only and known-only; expected 3.0 got {m}"
    );
}
