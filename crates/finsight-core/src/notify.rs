...
impl NotificationCategory {
    ...
    /// Every category, for building the preferences UI.
...
pub enum Disposition {
    ...
    Delivered,
    /// Recorded but held for quiet hours; surfaces in the in-app history only.
    Held,
...
pub fn load_prefs(conn: &Connection) -> Prefs {
    ...
    Prefs { master_enabled, disabled_categories, quiet_hours, utc_offset_minutes, privacy }
}
...
pub fn enqueue(
    ...
    let held = !matches!(new.urgency, Urgency::Critical)
    ...
    Ok(EnqueueOutcome {
        id: Some(id),
        disposition: if held { Disposition::Held } else { Disposition::Delivered },
        push: !held,
    })
}
