    ...
    fn build_system_prompt(tools: &ToolSet) -> String {
        ...
        format!(
            "You are a personal financial analyst for a local-first personal finance app.\n\
             ...
             CLARIFICATION BLOCK: when the ambiguity genuinely BLOCKS you — you cannot answer at all until you know which entity was meant — emit a single {{\"kind\":\"clarification\"}} block instead, and keep the prose answer to one line saying what you need. Set referenceType to the kind of thing you are asking about (\"account\" is supported today) and ALWAYS send \"options\":[] — the server fills the options from the user's real data. Never write options yourself: an invented account is one the user does not have, and clicking it would produce a confidently wrong answer. Use follow_up_questions for a question you could proceed without; use a clarification block only when you truly cannot.\n\
