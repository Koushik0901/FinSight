...
fn deterministic_copilot_fallback(
    ...
    if asks_goal_eta {
        let goals: Vec<(String,i64,i64)> = conn.prepare("SELECT name, target_cents, current_cents FROM goals WHERE archived_at IS NULL LIMIT 3").ok().map(|mut stmt| stmt.query_map([], |r| Ok((r.get::<_,String>(0)?, r.get::<_,i64>(1)?, r.get::<_,i64>(2)?))).ok().and_then(|rows| rows.collect::<Result<Vec<_>,_>>().ok()).unwrap_or_default()).unwrap_or_default();
        if goals.is_empty() {
    ...
    if asks_debt {
        let debts: Vec<(String,i64)> = conn.prepare("SELECT name, balance_cents FROM liabilities WHERE archived_at IS NULL ORDER BY balance_cents ASC LIMIT 5").ok().map(|mut stmt| stmt.query_map([], |r| Ok((r.get::<_,String>(0)?, r.get::<_,i64>(1)?))).ok().and_then(|rows| rows.collect::<Result<Vec<_>,_>>().ok()).unwrap_or_default()).unwrap_or_default();
        if debts.is_empty() {
