...
/// bindings.ts sends.
///
/// The leading idents (`api, events, cmd, p, c`) exist because `macro_rules!`
/// hygiene isolates definition-site names from the call-site arm bodies: they
/// are threaded through so both the generated signature and every arm refer
/// to the very same identifiers.
macro_rules! rpc_routes {
    ( $api:ident, $events:ident, $cmd:ident, $p:ident, $c:ident :
      $( $name:literal => $body:expr ),* $(,)? ) => {
        /// Every command with a route below. GENERATED by `rpc_routes!` from
        /// the same entries, so this list and the routes cannot drift. The
        /// Task 10 parity test proves this == (bindings.ts commands -
        /// UNSUPPORTED).
