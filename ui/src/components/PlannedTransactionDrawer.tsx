...
export default function PlannedTransactionDrawer({ open, onClose, planned }: Props) {
  const createPlanned = useCreatePlannedTransaction();
  ...
  async function save() {
    ...
    if (!description.trim() || !amount.trim() || !dueDate || Number.isNaN(amountCents)) {
    ...
    try {
      if (planned) {
        ...
        toast.success("Planned transaction added");
      }
      onClose();
    } catch (error) {
  ...
  return (
    <Drawer open={open} onClose={onClose} title={planned ? `Planned transaction · ${planned.description}` : "Add planned transaction"} width={560}>
      <div className="drawer-form">
        {planned && (
          <div className="card tight" style={{ padding: 16, background: "var(--surface-2)" }}>
            <div className="row row-sm wrap" style={{ marginBottom: 10 }}>
              ...
              <span className="chip">{formatCalendarDate(planned.dueDate, { month: "short", day: "numeric", year: "numeric" })}</span>
        ...
        <div className="form-actions">
          ...
          {planned && <button type="button" onClick={() => void remove()} disabled={deletePlanned.isPending}>Delete</button>}
          <button type="button" className="primary" onClick={() => void save()} disabled={createPlanned.isPending || updatePlanned.isPending}>
            {planned ? "Save transaction" : "Add transaction"}
          </button>
        </div>
      </div>
    </Drawer>
