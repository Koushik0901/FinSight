...
- **Native notification for background deep-answer completion (dropped in Task 6, controller-approved):** `spawn_deep_answer` used to fire a desktop notification when an async deep analysis finished. It cannot live in tauri-free finsight-api and cannot hoist to the wrapper (fires long after the wrapper returns). The `copilot-async-answer` event the UI renders from is fully preserved. Phase 4's thin shell needs a general events→native-notifications bridge anyway — restore it there.
