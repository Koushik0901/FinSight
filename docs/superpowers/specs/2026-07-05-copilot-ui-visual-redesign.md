...
**Validation & fallback (corrected against the actual pipeline):** the app already has a three-gate validation chain for artifacts, kept explicitly "in lockstep" by existing code comments, and each of the 7 new kinds must be added to all three:

1. `AgentResponseBlock` enum + `valid_response_block` in `crates/finsight-app/src/commands/agent.rs:508` — structural parse of the model's raw JSON (`parse_response_blocks`, `agent.rs:579`) plus first semantic check (non-empty/bounded).
2. `response_block_within_artifact_bounds` in `crates/finsight-app/src/commands/copilot_chat.rs:1126` — a second, stricter bounds gate (row/column/label caps) applied specifically before a block is allowed to become an interactive artifact; also add each new kind to `should_emit_response_block` (`copilot_chat.rs:1093`, decides which kinds render as artifacts at all — e.g. `Markdown` is deliberately excluded there since it's already carried by the prose text part).
3. `CopilotResponseBlockSchema` (Zod) in `ui/src/components/copilot/agUi/artifacts.ts:21` — the frontend mirrors the same bounds. Notably, **a stub already exists here**: `TransactionTablePropsSchema` (`artifacts.ts:65`) and a `TransactionTable` entry in `COMPONENT_PROP_SCHEMAS` (`artifacts.ts:76`), left by a prior session anticipating this exact feature, with the comment "the backend does not emit this yet." This confirms the intended shape and should be reconciled with the `TransactionTable` schema in §5 rather than duplicated.

**Rejection path:** in `TauriAgUiAgent.ts:293-310`, a `responseBlock` frame is serialized via `serializeFinanceArtifactEnvelope` (which re-validates through the same Zod schema) and wrapped as a synthetic `render_finance_artifact` tool call (`TOOL_CALL_START`/`TOOL_CALL_END`) — this is how `CopilotToolCard` in `renderers.tsx` ends up rendering `FinSightResponseBlock`, on **both** the AG-UI and legacy runtimes (both converge on this same synthetic-tool-call rendering path). If serialization/validation fails, it emits a `CUSTOM finsight.invalid_artifact` event instead and skips that one card — the answer's prose text still streams unconditionally afterward (`copilot_chat.rs`'s word-by-word streaming step runs regardless of artifacts), so a rejected artifact means one missing card, never a blank assistant bubble.
