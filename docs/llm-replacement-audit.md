...
---
**Decision for A1:** **Shipped centroid + FastText hybrid as default; LLM for low-conf only.** **2026-09-01: `Rule → fastText ≥0.6 → LLM` wired and enabled by default (`fasttext-local`, 39 MB model at `FINSIGHT_DATA_DIR/models/fasttext/merchant_ft.bin` with a development fallback). POC 95-96% acc, 6%→other, gap -0.3%, groceries 0.80 thin spot → ensemble lifts. Token cut ~90-95%. Retain `LOW_CONFIDENCE_THRESHOLD=0.6`.**
